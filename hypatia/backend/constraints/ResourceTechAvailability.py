                    ...
                    for indx, year in enumerate(self.model_data.settings.years):
                        ...
                        rules.append(
                            cp.sum(
                                ...
                                axis=0,)
                            - cp.multiply(
                                cp.multiply(cp.sum(available_prod, axis=0), self.model_data.regional_parameters[reg]["tech_availability"].loc[year, (key, slice(None))]),
                                self.model_data.regional_parameters[reg]["min_tech_capacity_factor"].loc[year, (key, slice(None))])
                            >= 0
                        )
