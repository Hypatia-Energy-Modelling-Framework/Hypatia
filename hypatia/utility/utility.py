...
def _calc_variable_overall(glob_technologies,regions,main_years,technologies,variable):
    ...
    variable_overall = {}
    for tech in list(glob_technologies.loc[glob_technologies['Tech_category'] != 'Demand']['Technology']):
...
def _calc_production_overall(glob_technologies,regions,main_years,technologies,variable):
    ...
    production_overall = {}
    for tech in list(glob_technologies.loc[(glob_technologies['Tech_category'] != 'Demand') & (glob_technologies['Tech_category'] != 'Storage')]['Technology']):
...
def line_newcap_accumulated(line_newcap,carriers,main_years,line_tlft):
    ...
    line_newcap_accumulated = cp.reshape(line_newacp_accumulated_reshape, line_newcap.shape)
    ...
    for tech in techs: 
        for indx,year in enumerate(main_years):
            ...
            try:
                ...
                decom_matrix.loc[(tech,main_years[int(indx+tlft[tech].values)]),(tech,year)] = 1
...
def line_decomcap(line_newcap,carriers,main_years,line_tlft):
    ...
    for carrier in carriers: 
        for year in main_years:
            ...
            try:
                decom_matrix_line.loc[(carrier,main_years[main_years.index(year)+line_tlft[carrier]]),(carrier,year)] = 1
...
def invcosts_annuity(cost_inv_present,interest_rate,economiclife,technologies,main_years,discount_rate):
    ...
    depreciation = np.divide(np.multiply(np.power((interest_rate.values +1),economiclife.values),
                           interest_rate.values),(np.power((interest_rate.values +1),economiclife.values)-1))
    depreciation = pd.DataFrame(depreciation,index=['Depreciation_rate'], columns = technologies)
    ...
    for tech_indx,tech in enumerate(technologies):
        ...
        for y_indx,year in enumerate(main_years):
            ...
            for future_year in range(y_indx + 1,
                                y_indx + economiclife.loc['Economic Life time',tech]+1):

                annuity = cost_inv_present[y_indx,tech_indx]*depreciation.loc['Depreciation_rate',tech]

                inv_fvalue_annual_discounted += annuity*(1+discount_rate.loc[year,'Annual Discount Rate'])**(-future_year)
...
def varcost(specific_varcost,activity,time_step):
    ...
    specific_varcost_reshape = pd.concat([specific_varcost]*len(time_step)).sort_index()
    variablecost = cp.multiply(specific_varcost_reshape.values , activity)
    
    return variablecost


def available_resource_prod(totalcap,capacity_factor,timeslice_fraction,annualprod_per_unitcapacity):
...
def annual_activity(activity,main_years,timeslices):
    ...
    for indx,year in enumerate(main_years[1:]):
        ...
        activity_annual_rest = cp.sum(activity[(indx+1)*len(timeslices):(indx+2)*len(timeslices),:],axis=0,keepdims=True)
...
def line_varcost(specific_varcost,line_import,regions,main_years,time_slices,lines_list):
    ...
    for reg in regions:
        ...
        for key,value in line_import[reg].items():
            ...
            if '{}-{}'.format(reg,key) in lines_list:
                ...
                specific_varcost_line = specific_varcost.loc[:,'{}-{}'.format(reg,key)]
                
            elif '{}-{}'.format(reg,key) in lines_list:
                
                specific_varcost_line = specific_varcost.loc[:,'{}-{}'.format(key,reg)]
                
            
            variablecost_line_regional[key] = cp.multiply(specific_varcost_line,line_import_anunual)
...
def salvage_factor(main_years,technologies,tlft,interest_rate,discount_rate,economiclife):
    ...
    for tech in technologies:
        ...
        technical_factor = (1-1/(1+interest_rate[tech].values))/(1-1/((1+interest_rate[tech].values)**\
                                                                                      economiclife[tech].values))
        
        social_factor = (1-1/((1+discount_rate.values)**economiclife[tech].values))/(1-1/(1+discount_rate.values))
        ...
        for indx,year in enumerate(main_years):
            ...
            if indx+tlft[tech].values > EOH:
                ...
                salvage_factor_0.loc[year,tech] = ((1+discount_rate.loc[year,:].values)**\
                                                  (tlft[tech].values-EOH-1+indx)-1)/((1+discount_rate.loc[year,:].values)\
                                                                                                                     **tlft[tech].values-1)
    
                                                                                                                     
    salvage_factor_mod = pd.DataFrame(salvage_factor_0.values*rates_factor.values,index=main_years,columns=technologies)
